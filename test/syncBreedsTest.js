...
describe('#breedDetails', () => {
  ...
  it('returns breed details for the street breed', () => {
    // We expect it to be a function, but will it be?
    const expectedDesc = '';
    const street = breedDetails('Street');
    assert.equal(expectedDesc, street);
  });
  it('should not return breed details for unknown breed', () => {
    // We expect it to be a function, but will it be?
    const expectedDesc = '';
    const unknown = breedDetails('Unknown');
    assert.notEqual(expectedDesc, unknown);
  });
});
